...
public class CK_RK implements CRE.RK {
	...
	public CK_RK(String table_name) {
		this.table_name = table_name;
		table_label = table_name.substring(2);
	}

	public JFrame run() {
		jf = new CK_RK(table_name).new CKF1();
		return jf;
	}
	...
	public class CKF1P0 extends F1P0 {
		...
		public void init() {
			...
			dmcv = new String[][] { SQLSupport.sqlFillDmc("CG_JH", "审核状态", VERIFIED, moreCond, new int[] { 1, 2, 3 }),
		...
		public void H() {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "当前没有可以关联的采购单，请联系采购部门。");
			}
		...
		public void updateDmc(MyComboBox[] dmc, String updItem) {
			...
			String repo = UtilSupport.substringBetween(updItem, ",", ")");
			// 设置dmc1（仓库与单号的仓库对应）
			JComboBox<Object> dmc1 = dmc[1].getDmc();
			dmc1.setSelectedIndex(UtilSupport.filterOrder(dmc[1].getsValue(), (s) -> s.contains(repo)));
			// 填充1~8列（药品属性列）
			Object obj = SQLSupport.sqlGetMultiValue("YP", "CG_JH_X", "药品编号", "单号", id);
			...
			for (int i = 0; i < dtm.getRowCount(); i++) {
				...
				String[] sValue = SQLSupport.sqlGetValue("CG_JH_X", "单号", id,
						new String[] { mtb.getColN()[9], mtb.getColN()[10] }, moreCond);
				dtm.setValueAt(sValue[0], i, 9);
				dtm.setValueAt(sValue[1], i, 10);
			}
			// 填充金额区域
			String[] $ = SQLSupport.sqlGetValue("CG_JH", "单号", id, new String[] { "应付金额", "实付金额", "待付金额" }, "");
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 保存
				...
				SQLSupport.sqlUpdate("CG_JH", "关联状态", ASSOCIATED, "单号", s2, "");
	...
	public class CKF1P1 extends F1P1 {
	...
	public class CKF2P0 extends F2P0 {
		...
		public void H() {
			...
			// 填充0~7列（药品属性列）
			dtm.addDual(SQLSupport.sqlGetMultiValue("YP", "CG_JH_X", "药品编号", "单号", F1P1.asso_label));
			// 填充8~9列（金额列）
			for (int i = 0; i < dtm.getRowCount(); i++) {
				String moreCond = String.format("and %s = '%s'", "药品编号", dtm.getValueAt(i, 0).toString().trim());
				String[] sValue = SQLSupport.sqlGetValue("CG_JH_X", "单号", F1P1.asso_label,
