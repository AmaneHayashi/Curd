...
public class XS_DH implements CSE.DH {
	...
	public XS_DH(String table_name) {
	...
	@Override
	public JFrame run() {
		jf = new XS_DH(table_name).new XSF1();
		return jf;
	}
	...
	public class XSF1P0 extends F1P0 {
		...
		public void init() {
			...
			mt2s = F1P0mt2s;
			dmcs = F1P0dmcs;
			dmcv = new String[][] { SQLSupport.sqlFillDmc("KH", "1", "1", "", new int[] { 0, 1 }), // diff1
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 保存
				...
				SQLSupport.sqlInsert(table_name, new String[] { s0, s1, s2, s3, s4, mt1[0].S(), mt1[1].S(), mt1[2].S(),
						mt1[3].S(), mt2.S(), VERIFING, CHECKOUTING });
			...
			if (e.getSource().equals(jb1[0])) {
				if (selectRow == -1) {
					...
					try {
						...
						} else if (assoValue.equals(CHECKOUTING)) {
							JOptionPane.showMessageDialog(null, "本单据等待仓库出库，请与仓库管理员联系。");
						} else {
	...
	public class XSF2P0 extends F2P0 {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource().equals(jb[0])) {
				...
				// 先前应收金额
				float f2 = Float.parseFloat(SQLSupport.sqlGetUniqueValue(table_name, "单号", sl, "应收金额", ""));
				// 先前已收金额
				float f3 = Float.parseFloat(SQLSupport.sqlGetUniqueValue(table_name, "单号", sl, "实收金额", ""));
				System.out.println("当前应收：" + f1 + "，先前应收：" + f2 + "，先前已收：" + f3);
				if (f1 >= f3) {
					SQLSupport.sqlUpdate(table_name, "应收金额", UtilSupport.scaleF(f1, FLOAT_SCALE), "单号", sl, "");
					SQLSupport.sqlUpdate(table_name, "待收金额", UtilSupport.scaleF(f1 - f3, FLOAT_SCALE), "单号", sl, "");
				} else if (f1 <= f2) {
					SQLSupport.sqlUpdate(table_name, "应收金额", UtilSupport.scaleF(f1, FLOAT_SCALE), "单号", sl, "");
					SQLSupport.sqlUpdate(table_name, "实收金额", UtilSupport.scaleF(f1, FLOAT_SCALE), "单号", sl, "");
					SQLSupport.sqlUpdate(table_name, "待收金额", UtilSupport.scaleF(0, FLOAT_SCALE), "单号", sl, "");
				}
				// 更新药品数量与金额
